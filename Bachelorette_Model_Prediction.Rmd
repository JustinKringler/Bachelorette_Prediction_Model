...
I have decided to do my modeling based off the hit reality game show, "The Bachelorette". The goal is to use two data sources to create a classifcation model using seasons 5-12 to predict who will win and then test it on a future season. We chose seasons 5-12 because previous season had different rules which would  ruin the consistency with our outcome. 
...
We have to replace the names manually because they were spelt wrong so there was an error on the other end for data entry. 
...
Overeall I am happy with the outcome of our model and feel confident we chose one of the best choices. The model is not perfect and the confidence interval is too large but other than that it is a decent model.
